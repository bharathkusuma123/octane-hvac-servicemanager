...
const CustomerView = () => {
  ...
  useEffect(() => {
    const fetchCustomer = async () => {
      try {
        ...
        if (response.data.status === "success") {
          setCustomer(response.data.data);
          // Fetch service items for this customer
          fetchServiceItems(response.data.data.customer_id);
        } else {
          setError("Customer not found");
        }
      } catch (error) {
  ...
  // Function to fetch service items for this customer
  const fetchServiceItems = async (customerId) => {
    try {
      ...
      if (response.data && response.data.data) {
        // Filter service items by customer ID
        const customerServiceItems = response.data.data.filter(
          item => item.customer === customerId
        );
        setServiceItems(customerServiceItems);
      }
    } catch (error) {
  ...
  // Function to navigate to service item details
  const handleViewServiceItem = (serviceItemId) => {
                ...
                <tr>
                  ...
                  <td>
                    {serviceItems.length > 0 ? (
                      <div className="d-flex flex-wrap gap-2">
                        {serviceItems.map((item, index) => (
                          <button 
                            ...
                            onClick={() => handleViewServiceItem(item.service_item_id)}
                            style={{
                              color: '#0d6efd',
                            ...
                            title={`View Service Item: ${item.service_item_id}`}
                          >
                      ...
                      <span className="text-muted">No service items found</span>
                    )}
                  </td>
                </tr>
              </tbody>
