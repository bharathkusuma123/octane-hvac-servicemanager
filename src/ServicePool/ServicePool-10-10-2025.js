...
useEffect(() => {
  const fetchAssignmentHistory = async () => {
    try {
      ...
      // Sort history data by created_at in descending order (newest first)
...
const handleReopenService = async (item) => {
  const result = await Swal.fire({
    ...
    text: `Are you sure you want to re-open request ${item.request_id}? This will create a new service request that can be assigned to a different engineer.`,
  ...
  if (result.isConfirmed) {
    try {
      ...
      // Prepare the payload for the new reopened request
      const payload = {
        // Required fields
        source_type: 'Re-Opened',
        status: 'Open', // Start as Open so it can be assigned
        company: item.company,
        service_item: item.service_item?.id || item.service_item,
        customer: item.customer?.id || item.customer,
        
        // Copy relevant details from original request
        ...
        // Set new preferred date/time (current or future)
        preferred_date: new Date().toISOString().split('T')[0], // Today
        preferred_time: '09:00:00', // Default morning time
        
        // Reset assignment-related fields since this is a new request
        ...
        // Link to the original request that's being reopened
      ...
      // Step 1: Create the new reopened service request
      ...
      // Step 2: Update the original request status to "Reopened"
      try {
        await axios.put( // Using PATCH to update only specific fields
          `${baseURL}/service-pools/${item.request_id}/`,
          {
            status: 'Reopened',
            updated_by: userId,
            user_id: userId,
        ...
        console.log(`Original request ${item.request_id} status updated to Reopened`);
      } catch (updateError) {
        console.warn('Note: Original request status update failed, but new request was created:', updateError);
        // Continue anyway since the main operation (reopening) succeeded
      }
      ...
      // Refresh the data to show both the updated original and new request
      await fetchPoolData();
  ...
  return (
    <div className="service-container pm-container">
      ...
      // Find the LATEST assignment history for this request
