...
const ServiceContractForm = () => {
    ...
    useEffect(() => {
        if (contractData.end_date && contractData.alert_days) {
            const newAlertDate = calculateAlertDate(contractData.end_date, contractData.alert_days);
            setContractData(prev => ({ ...prev, alert_date: newAlertDate }));
        }
    }, [contractData.end_date, contractData.alert_days]);

    useEffect(() => {
        if (contractData.end_date && contractData.overdue_alert_days) {
            const newOverdueDate = calculateOverdueAlertDate(contractData.end_date, contractData.overdue_alert_days);
            setContractData(prev => ({ ...prev, overdue_alert_date: newOverdueDate }));
        }
    }, [contractData.end_date, contractData.overdue_alert_days]);
