...
const ServiceItemDetails = () => {
    ...
    const fetchReferenceData = async () => {
      try {
        ...
        const companiesRes = await axios.get(
          `${baseURL}/companies/?user_id=${userId}&company_id=${selectedCompany}`
        );
        ...
        const productsRes = await axios.get(
          `${baseURL}/products/?user_id=${userId}&company_id=${selectedCompany}`
        );
        ...
        // Fetch PM Groups
        const pmGroupsRes = await axios.get(
          `${baseURL}/pm-groups/?user_id=${userId}&company_id=${selectedCompany}`
        );
        if (pmGroupsRes.data?.status === "success") {
          setPmGroups(pmGroupsRes.data.data || []);
        }
      } catch (error) {
  ...
  const getCustomerName = (customerId) => {
    ...
    if (customer) {
      return customer.full_name || customer.username || customerId;
    }
    return customerId;
  };

  const getPmGroupName = (pmGroupId) => {
    if (!pmGroupId) return 'N/A';
    const pmGroup = pmGroups.find(p => p.pm_group_id === pmGroupId);
                  ...
                  <tr>
                    ...
                    <td>
                      <div className="d-flex flex-column">
                        <span>{getCompanyName(serviceItem.company)}</span>
                        {/* <small className="text-muted">ID: {serviceItem.company}</small> */}
                      </div>
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                      <div className="d-flex flex-column">
                        <span>{getProductName(serviceItem.product)}</span>
                        {/* <small className="text-muted">ID: {serviceItem.product}</small> */}
                      </div>
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                      <div className="d-flex flex-column">
                        <span>{getCustomerName(serviceItem.customer)}</span>
                        {/* <small className="text-muted">ID: {serviceItem.customer}</small> */}
                      </div>
                    </td>
                  </tr>
                  <tr>
                    ...
                    <td>
                      <div className="d-flex flex-column">
                        <span>{getPmGroupName(serviceItem.pm_group)}</span>
                        {/* <small className="text-muted">ID: {serviceItem.pm_group}</small> */}
                      </div>
                    </td>
                  </tr>
                  <tr>
